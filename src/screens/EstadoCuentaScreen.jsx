    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 py-3 bg-white border-b border-gray-200">
        ...
        <select value={cliente} onChange={(e) => setCliente(e.target.value)}
          className="flex-1 border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400">
      ...
      {filtradas.length > 0 && (
        <div className="flex gap-6 px-6 py-2 bg-green-50 border-b border-amber-300 text-sm">
          ...
          <span className="text-amber-700">Cobrado: <strong>{fmtMoney(totP, mon)}</strong></span>
          <span className={`font-bold ${totB - totP > 0 ? "text-red-600" : "text-amber-700"}`}>
                ...
                return (
                  <React.Fragment key={d.id}>
                    <tr className="cursor-pointer" onClick={() => setExpanded(isExp ? null : d.id)}>
                      ...
                      <td className="text-amber-700">{fmtMoney(d.pagado || 0, dMon)}</td>
                      <td className={`font-bold ${saldo > 0 ? "text-red-600" : "text-amber-700"}`}>{fmtMoney(saldo, dMon)}</td>
