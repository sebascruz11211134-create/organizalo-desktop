      ...
      <div className="px-8 py-6 space-y-6 max-w-4xl">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            ["Crédito fiscal (compras)", fmtMoney(creditoFiscal,"CRC"), "text-amber-700"],
          ].map(([lbl,val,cls])=>(
            ...
            <tbody>
              {compMes.length === 0 ? (
                ...
                <tr key={c.id}>
                  ...
                  <td className="text-right font-semibold text-amber-700">{fmtMoney(parseFloat(c.montoImpuesto||c.ivaTotal||0),"CRC")}</td>
                </tr>
              ))}
              {compMes.length>0 && (
                <tr className="bg-green-50 font-bold border-t-2 border-amber-300">
        ...
        {/* Resultado D-104 */}
        <div className={`rounded-xl p-6 border-2 ${saldo>0 ? "border-red-300 bg-red-50" : "border-amber-300 bg-green-50"}`}>
          <div className="flex items-center gap-3">
            {saldo > 0
              ? <AlertTriangle size={24} className="text-red-600 shrink-0"/>
              : <CheckCircle size={24} className="text-amber-600 shrink-0"/>}
            <div>
              <p className={`text-lg font-black ${saldo>0?"text-red-700":"text-amber-700"}`}>
                {saldo > 0
              ...
              <p className={`text-sm mt-0.5 ${saldo>0?"text-red-600":"text-amber-600"}`}>
