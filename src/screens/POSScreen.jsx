      ...
      <div className="flex flex-col items-center justify-center h-full gap-5">
        ...
        <div className="text-center">
          ...
          {mensaje.medio==="Efectivo" && mensaje.cambio>0 && (
            <p className="text-lg font-bold text-green-700 mt-2">Cambio: {fmtMoney(mensaje.cambio,"CRC")}</p>
          )}
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={()=>setMensaje(null)} className="flex items-center gap-2 bg-brand-500 text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-brand-600">
        ...
        <div className="p-4 border-b border-slate-200 bg-white">
          <div className="relative">
            ...
            <input ref={inputRef} value={busq} onChange={e=>setBusq(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {filtrados.map(p => (
              <button key={p.id} onClick={()=>agregar(p)}
                className="bg-white border border-slate-200 rounded-xl p-3 text-left hover:border-brand-400 hover:shadow-sm transition-all active:scale-95">
                <div className="w-8 h-8 rounded-lg bg-brand-50 flex items-center justify-center mb-2">
                  <span className="text-brand-600 font-bold text-sm">{(p.nombre||"?").charAt(0)}</span>
                </div>
                <p className="text-sm font-semibold text-slate-800 truncate leading-tight">{p.nombre}</p>
                <p className="text-xs font-bold text-green-700 mt-0.5">{fmtMoney(p.precio||0,"CRC")}</p>
          ...
          {carrito.length===0 ? (
            ...
            <div key={x.id} className="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-green-700">{fmtMoney((x.precio||0)*x.cant,"CRC")}</p>
              </div>
        ...
        <div className="p-4 border-t border-slate-200 space-y-3">
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between font-black text-lg text-slate-900 border-t border-slate-200 pt-2">
              <span>Total</span><span className="text-green-700">{fmtMoney(total,"CRC")}</span>
            </div>
          </div>
          ...
          <div className="flex gap-1">
            {MEDIOS.map(m=>(
              <button key={m} onClick={()=>setMedio(m)}
                className={`flex-1 py-1.5 text-[10px] font-semibold rounded-lg border transition-all ${medio===m?"bg-brand-500 text-white border-brand-500":"border-slate-200 text-slate-500 hover:bg-slate-50"}`}>
                {m}
              </button>
            ))}
          </div>
          ...
          {medio==="Efectivo" && (
            <div>
              ...
              <input type="number" value={efectivo} onChange={e=>setEfectivo(e.target.value)}
                placeholder="0"
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-right focus:outline-none focus:ring-1 focus:ring-brand-400" />
              {parseFloat(efectivo)>0 && (
                <p className="text-xs text-right text-green-600 mt-1 font-semibold">Cambio: {fmtMoney(cambio,"CRC")}</p>
              )}
            </div>
          )}

          <button onClick={cobrar} disabled={carrito.length===0}
            className="w-full bg-brand-500 text-white py-3 rounded-xl font-bold text-sm hover:bg-brand-600 disabled:opacity-40 transition-all active:scale-95">
