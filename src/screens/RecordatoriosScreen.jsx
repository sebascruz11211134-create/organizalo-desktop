          ...
          <div className="grid grid-cols-4 gap-4">
            <label className="block">
              ...
              <input type="number" min="0" max="90" value={config.diasAviso} onChange={e=>setConfig(p=>({...p,diasAviso:parseInt(e.target.value)||7}))}
                className="mt-1 w-full border border-slate-200 rounded-md px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-brand-400"/>
            </label>
            <label className="block">
            ...
            <div className="flex items-end">
              <button onClick={guardarConfig} className="w-full bg-brand-500 text-white py-2 rounded-lg text-sm font-medium hover:bg-brand-600">
                Guardar config
              </button>
            </div>
          </div>
      ...
      <div className="flex gap-2 px-8 py-3 bg-white border-b border-slate-100">
        {[["todos","Todos"],["vencidas","Vencidas"],["proximas","Próximas"]].map(([k,lbl])=>(
          <button key={k} onClick={()=>setFiltro(k)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${filtro===k?"bg-brand-500 text-white":"text-slate-500 hover:bg-slate-100"}`}>
            {lbl}
          </button>
        ))}
      </div>
            ...
            <div key={d.id} className={`bg-white border rounded-xl px-5 py-4 flex items-center gap-4
              ...
              <button onClick={()=>enviarWhatsApp(d)}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold shrink-0 transition-all
                  ${ya?"bg-green-50 text-green-700 border border-green-200":"bg-green-600 text-white hover:bg-green-700"}`}>
