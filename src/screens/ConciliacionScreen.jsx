...
function TabImportarCSV() {
  ...
  return (
    <div className="flex flex-col h-full">
      {filas.length===0 ? (
        <div className={`flex-1 flex flex-col items-center justify-center gap-4 m-6 border-2 border-dashed rounded-2xl transition-colors ${arrastrando?"border-brand-400 bg-brand-50":"border-slate-200 bg-slate-50"}`}
          ...
          <label className="flex items-center gap-2 bg-brand-500 text-white px-5 py-2 rounded-lg text-sm font-semibold cursor-pointer hover:bg-brand-600">
            <Upload size={14}/> Seleccionar archivo CSV
            <input type="file" accept=".csv,.txt" className="hidden" onChange={cargarArchivo}/>
          </label>
          {resultado && (
            <div className={`px-4 py-2 rounded-lg text-sm font-semibold ${resultado.ok?"bg-green-100 text-green-800":"bg-red-100 text-red-700"}`}>
        ...
        <div className="flex flex-col h-full overflow-hidden">
          <div className="flex items-center gap-3 px-4 py-2 bg-slate-50 border-b text-xs">
            ...
            <button onClick={importar} disabled={guardando}
              className="ml-auto bg-brand-500 text-white px-4 py-1.5 rounded-lg text-xs font-semibold hover:bg-brand-600 disabled:opacity-50">
                  ...
                  <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-1.5 text-center">
                      <button onClick={()=>toggleTipo(i)}
                        className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${f.tipo==="ingreso"?"bg-green-100 text-green-700":"bg-red-100 text-red-600"}`}>
