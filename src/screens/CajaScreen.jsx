    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <label className="block">
            ...
            <div className="flex gap-2 mt-1">
              <button onClick={()=>u("esIngreso",true)} className={`flex-1 py-2 rounded-lg text-sm font-semibold border-2 transition-colors ${form.esIngreso?"border-green-600 bg-green-50 text-green-700":"border-slate-200 text-slate-500"}`}>↑ Ingreso</button>
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button onClick={guardar} className="flex-1 bg-brand-500 text-white py-2 rounded-lg text-sm font-medium hover:bg-brand-600">Agregar</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          {BILLETES.map(b=>(
            <div key={b} className="flex items-center gap-3">
              ...
              <input type="number" min="0" value={conteo[b]||""} onChange={e=>u(b,e.target.value)}
                className="w-20 border border-slate-200 rounded-md px-2 py-1.5 text-sm text-center focus:outline-none focus:ring-1 focus:ring-brand-400"/>
        ...
        <div className="mt-5 space-y-2 border-t border-slate-200 pt-4">
          ...
          <div className={`flex justify-between text-sm font-bold border-t border-slate-200 pt-2 ${diferencia===0?"text-green-700":diferencia>0?"text-blue-700":"text-red-600"}`}>
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button onClick={()=>onCerrar({ conteo, totalFisico, diferencia })}
            className="flex-1 bg-brand-500 text-white py-2 rounded-lg text-sm font-medium hover:bg-brand-600">
            Confirmar cierre
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-0 border-b border-slate-200 bg-white text-center">
            {[
              ...
              ["Ingresos",          fmtMoney(totalIngresos,"CRC"),         "text-green-700"],
          ...
          {cerrada && cajaDia.arqueo && (
            <div className={`flex items-center gap-3 px-6 py-3 border-b text-sm font-semibold
              ${cajaDia.arqueo.diferencia===0?"bg-green-50 border-green-100 text-green-800":cajaDia.arqueo.diferencia>0?"bg-blue-50 border-blue-100 text-blue-800":"bg-red-50 border-red-100 text-red-700"}`}>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={()=>setModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={abrirCaja} className="flex-1 bg-brand-500 text-white py-2 rounded-lg text-sm font-medium hover:bg-brand-600">Abrir caja</button>
            </div>
          </div>
        </div>
      )}
