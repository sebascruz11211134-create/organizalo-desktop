    ...
    <div className="grid grid-cols-2 gap-6 max-w-4xl">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <Section titulo="Patrimonio"
          ...
          total={patrimonio}/>
        <div className={`flex justify-between font-black text-base border-t-2 mt-2 pt-2 ${balanceado?"border-green-700 text-green-800":"border-red-500 text-red-700"}`}>
          ...
          {TABS.map(t=>(
            <button key={t} onClick={()=>setTab(t)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors
                ${tab===t?"bg-brand-500 text-white":"text-slate-500 hover:text-slate-800 hover:bg-slate-100"}`}>
              {t}
            </button>
          ))}
        </div>
