      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[80vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-3">
          {["facturas","gastos"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${tab===t?"bg-amber-600 text-white":"bg-slate-100 text-slate-500"}`}>
        ...
        <div className="flex gap-3 mt-4 border-t border-slate-100 pt-4">
          <button onClick={onClose} className="flex-1 border border-gray-200 text-slate-600 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50">Cancelar</button>
          <button onClick={guardar}  className="flex-1 bg-amber-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-amber-700">Asignar seleccionados</button>
        </div>
      </div>
    </div>
  );
...
export default function ProyectosScreen() {
  ...
  const estadoColor = {
    "Activo":"text-amber-700 bg-amber-50", "Completado":"text-blue-700 bg-blue-50",
  ...
  return (
    <div className="flex flex-col h-full overflow-auto bg-slate-50">
      <div className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between">
        ...
        <button onClick={()=>setModal({type:"nuevo"})} className="flex items-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-amber-700">
              ...
              <div className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="flex gap-5 shrink-0">
                  <div className="text-right">
                    ...
                    <p className="text-sm font-semibold text-amber-700">{fmtMoney(ingresos,"CRC")}</p>
                  </div>
                  <div className="text-right">
                    ...
                    <div>
                      ...
                      {facturas.filter(f=>asig[f.id]==="facturas").slice(0,5).map(f=>(
                        <div key={f.id} className="flex justify-between text-xs text-slate-600 py-0.5">
                          ...
                          <span className="font-semibold text-amber-700">{fmtMoney(f.totalGeneral,"CRC")}</span>
                        </div>
                      ))}
                    </div>
                    {/* Gastos asignados */}
