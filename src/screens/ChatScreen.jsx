    ...
    return (
      <div className="flex flex-col mb-4 items-start">
        <div className="flex items-center gap-1.5 mb-1 ml-1">
          ...
          <span className="text-xs font-semibold text-yellow-700">Asistente Monki</span>
          <span className="text-[10px] bg-yellow-100 text-yellow-600 px-1.5 py-0.5 rounded-full font-medium">IA</span>
        </div>
        <div className="max-w-sm lg:max-w-lg px-4 py-3 rounded-2xl rounded-tl-sm text-sm bg-yellow-50 border border-yellow-200 text-slate-800 whitespace-pre-wrap">
          {msg.texto}
        </div>
  ...
  return (
    <div className={`flex flex-col mb-3 ${esPropio ? "items-end" : "items-start"}`}>
      ...
      <div className={`max-w-xs lg:max-w-md px-3 py-2 rounded-2xl text-sm ${
        esPropio ? "bg-yellow-600 text-white rounded-br-sm" : "bg-slate-100 text-slate-800 rounded-bl-sm"
      }`}>
        {msg.texto}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        ...
        {!link ? (
          <div className="space-y-3">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                ...
                className="mt-1 w-full border border-slate-200 rounded px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-yellow-400"/>
            </div>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={invitar} disabled={loading}
                className="flex-1 bg-yellow-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-yellow-700 disabled:opacity-60">
          ...
          <div className="space-y-4">
            <div className="bg-green-50 border border-yellow-300 rounded-xl p-4">
              <p className="text-sm text-green-800 font-semibold mb-2">✅ Invitación generada</p>
              <p className="text-xs text-yellow-700 mb-3">Comparte este link con {email}. Expira en 7 días.</p>
              <div className="bg-white border border-yellow-300 rounded-lg px-3 py-2 text-xs text-slate-600 break-all">{link}</div>
            </div>
            <button onClick={copiar}
              className="w-full flex items-center justify-center gap-2 bg-yellow-600 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-yellow-700">
    ...
    <div className="flex h-full bg-white">
      ...
      <div className="w-56 flex-shrink-0 bg-slate-900 flex flex-col">
        ...
        <div className="px-4 py-3 border-t border-slate-700 flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-yellow-600 flex items-center justify-center text-white text-xs font-bold">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Banner soporte IA */}
        {canalActivo === "soporte" && (
          <div className="mx-6 mt-3 mb-1 px-4 py-2.5 bg-yellow-50 border border-yellow-200 rounded-xl flex items-center gap-2">
            <span className="text-lg">🤖</span>
            <div>
              <p className="text-xs font-semibold text-yellow-800">Asistente Monki — IA 24/7</p>
              <p className="text-xs text-yellow-600">Escribí tu duda o problema y te respondo al instante.</p>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {iaEscribiendo && canalActivo === "soporte" && (
            <div className="flex items-center gap-2 mb-3 ml-1">
              <span className="text-base">🤖</span>
              <div className="flex items-center gap-1 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-2xl rounded-tl-sm">
        ...
        <div className="px-4 pb-4">
          <div className="flex items-end gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2">
            ...
            <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-yellow-600 text-white hover:bg-yellow-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex-shrink-0"
            >
              <Send size={14}/>
            </button>
          </div>
