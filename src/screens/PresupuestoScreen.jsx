      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {editado && (
              <button onClick={guardar} className="flex items-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-amber-700">
        ...
        {MESES.map((m,i)=>(
          <button key={i} onClick={()=>setMesVista(i)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors
              ${mesVista===i?"bg-amber-600 text-white":"text-slate-500 hover:bg-slate-100"}`}>
            {m}
          </button>
        ))}
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                    ...
                    <tr key={c.codigo} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <input
                          ...
                          placeholder="0"
                          className="w-28 border border-slate-200 rounded px-2 py-1 text-sm text-right focus:outline-none focus:ring-1 focus:ring-amber-400"/>
                      </td>
                      <td className="px-4 py-2.5 text-right font-semibold text-slate-700">{fmtMoney(real,"CRC")}</td>
                      <td className={`px-4 py-2.5 text-right font-bold ${presp===0?"text-slate-400":ok?"text-amber-700":"text-red-600"}`}>
                        {presp===0?"—":`${var_>=0?"+":""}${fmtMoney(var_,"CRC")}`}
                      </td>
                      <td className="px-4 py-2.5 text-right">
                        {pct!==null ? (
                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${parseInt(pct)>=80?"bg-green-50 text-amber-700":parseInt(pct)>=50?"bg-yellow-50 text-yellow-700":"bg-red-50 text-red-600"}`}>
                            {pct}%
                          </span>
                ...
                <tr className="border-t-2 border-slate-300 bg-slate-50 font-bold">
                  ...
                  <td className={`px-4 py-3 text-right ${varianza>=0?"text-amber-700":"text-red-600"}`}>
                    {varianza>=0?"+":""}{fmtMoney(varianza,"CRC")}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {totalPresupMes>0 && (
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${(totalRealMes/totalPresupMes)>=0.8?"bg-green-50 text-amber-700":"bg-red-50 text-red-600"}`}>
