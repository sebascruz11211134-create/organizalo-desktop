                ...
                {resumen.map(({ emp, diasTrabajados, horasTotales, abierto }) => (
                  <tr key={emp.id}>
                    ...
                    <td className="text-center">
                      {abierto
                        ? <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-[10px] font-semibold">Trabajando</span>
                  ...
                  .map(r => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td className="text-emerald-600 font-mono text-xs">{r.entrada}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={()=>setModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={guardarManual} className="flex-1 py-2.5 bg-emerald-700 text-white rounded-lg text-sm font-semibold">Guardar</button>
            </div>
          </div>
        </div>
      )}
