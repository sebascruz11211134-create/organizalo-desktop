      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                ...
                <input type="number" value={f.precio} onChange={u("precio")} min="0" step="any" placeholder="0"
                  className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400 text-right" />
              </div>
              <div>
            ...
            {margen && (
              <p className="text-xs text-amber-600 font-semibold">Margen: {margen}% · Precio con IVA: {fmtMoney(parseFloat(f.precio)*(1+f.pctIVA/100),"CRC")}</p>
            )}
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                ...
                <input type="number" value={f.stock} onChange={u("stock")} min="0" placeholder="—"
                  className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400 text-center" />
              </div>
              <div>
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-3 flex-wrap">
        <div className="relative">
          ...
          <input value={busq} onChange={e=>setBusq(e.target.value)} placeholder="Buscar producto, código…"
            className="pl-8 pr-3 py-1.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-amber-400 w-56" />
        </div>
          ...
          <div className="flex border border-slate-200 rounded-lg overflow-hidden">
            {["grid","tabla"].map(v=>(
              <button key={v} onClick={()=>setVista(v)}
                className={`px-2.5 py-1 text-xs font-semibold transition-colors ${vista===v?"bg-amber-600 text-white":"text-slate-500 hover:bg-slate-50"}`}>
                {v==="grid"?"⊞":"☰"}
              </button>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        {filtrados.length===0 ? (
          ...
          <div className="grid grid-cols-4 gap-4">
            {filtrados.map(p => (
              <div key={p.id} className="bg-white border border-slate-200 rounded-xl p-4 hover:border-amber-300 hover:shadow-sm group transition-all">
                <div className="w-12 h-12 rounded-xl bg-amber-50 flex items-center justify-center mb-3">
                  <span className="text-amber-700 font-black text-xl">{(p.nombre||"?").charAt(0)}</span>
                </div>
                ...
                <p className="text-base font-black text-amber-700">{fmtMoney(p.precio||0,"CRC")}</p>
                {p.stock!=null && (
            ...
            <tbody className="divide-y divide-slate-100">
              {filtrados.map(p=>(
                <tr key={p.id} className="hover:bg-slate-50 group">
                  ...
                  <td className="px-4 py-2.5 text-right font-bold text-amber-700">{fmtMoney(p.precio||0,"CRC")}</td>
