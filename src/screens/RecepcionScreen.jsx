...
const ESTADO_BADGE = {
  ...
  aceptada:         { label: "Aceptada",         cls: "bg-green-100 text-emerald-700" },
    ...
    <div className="p-6 fade-in">
      ...
      <div
        ...
        className={`mb-5 border-2 border-dashed rounded-xl px-6 py-10 text-center cursor-pointer transition-colors
          ${dragging ? "border-emerald-300 bg-green-50" : "border-gray-300 hover:border-emerald-300 hover:bg-emerald-50"}`}>
        {subiendo ? (
          ...
          <div className="flex flex-col items-center gap-2 text-slate-400">
            ...
            <p className="text-sm font-medium text-slate-600">
              Arrastrá archivos <span className="font-mono text-emerald-700">.xml</span> aquí, o hacé click para seleccionarlos
            </p>
      ...
      {msg && (
        <div className={`mb-4 flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm
          ${msg.type === "ok" ? "bg-green-50 border border-emerald-300 text-emerald-700" : "bg-red-50 border border-red-200 text-red-700"}`}>
      ...
      <div className="mb-4 flex gap-3 items-end">
        <div className="flex-1">
          ...
          <input type="password" value={haciendaToken} onChange={(e) => setHaciendaToken(e.target.value)}
            ...
            className="w-full border border-gray-200 rounded px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-400" />
        </div>
        ...
        {pendientes > 0 && (
          <button onClick={aceptarTodos} disabled={aceptTodos}
            className="flex items-center gap-2 px-4 py-2 bg-emerald-700 text-white rounded-lg text-sm font-semibold hover:bg-green-800 disabled:opacity-50">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {cargando ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-2">
            ...
            <button onClick={cargar} className="mt-2 text-xs text-emerald-700 underline">Reintentar</button>
          </div>
        ) : facturas.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-2">
            <FileText size={36} />
                      ...
                      {esPend ? (
                        <div className="flex items-center justify-center gap-1">
                          <button
                            ...
                            disabled={proc}
                            title="Aceptar"
                            className="flex items-center gap-1 px-2.5 py-1 bg-green-100 text-emerald-700 rounded-lg text-xs font-semibold hover:bg-green-200 disabled:opacity-50">
