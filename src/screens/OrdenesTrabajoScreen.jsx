...
const ESTADOS = {
  ...
  listo:      { label:"Listo",       cls:"bg-green-100 text-amber-700" },
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input type="number" value={f.total||((parseFloat(f.manoObra)||0)+(parseFloat(f.repuestos)||0))} onChange={u("total")} min="0"
                className="w-full border border-slate-200 rounded px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400 text-right font-bold"/>
            </div>
          </div>
                ...
                <div className="absolute top-full left-0 w-full bg-white border border-slate-200 rounded-lg shadow-lg z-10 max-h-36 overflow-auto">
                  {prodsFilt.map(p=>(
                    <button key={p.id} onMouseDown={()=>agregarMaterial(p)}
                      className="w-full text-left px-3 py-2 text-xs hover:bg-amber-50 border-b last:border-0 flex justify-between">
      ...
      <div className="flex-1 overflow-auto p-6">
        {filtradas.length===0 ? (
          ...
          <div className="space-y-2">
            {filtradas.map(o=>(
              <div key={o.id} className="bg-white border border-slate-200 rounded-xl px-5 py-3.5 flex items-center gap-4 hover:border-amber-300 group">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100">
                  <button onClick={()=>facturarOT(o)}
                    className="flex items-center gap-1 text-xs px-2 py-1 rounded hover:bg-amber-50 text-amber-600 font-semibold">
