      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                  ...
                  return (
                    <div key={d.id} className="flex items-center gap-2 px-3 py-2 border-b last:border-0 bg-white hover:bg-slate-50">
                      ...
                      <input
                        ...
                        className="w-28 border border-slate-200 rounded px-2 py-1 text-xs text-right focus:outline-none focus:ring-1 focus:ring-amber-400"
                      />
                    </div>
                  );
                })}
                    ...
                    return (
                      <button key={f.id} type="button"
                        ...
                        className={`w-full text-left flex items-center gap-2 px-3 py-2 border-b last:border-0 transition-colors
                          ${isSel ? "bg-green-50 border-l-4 border-amber-300" : "bg-white hover:bg-slate-50"}`}>
                        <span className={`font-mono font-bold text-sm ${isSel ? "text-amber-700" : "text-slate-600"}`}>#{f.numero}</span>
                        <span className="text-xs text-slate-500 flex-1 truncate">{fmtDate(f.fecha)}</span>
                        <span className={`text-xs font-bold ${isSel ? "text-amber-700" : "text-slate-700"}`}>{fmtMoney(f.total, f.moneda)}</span>
                        {isSel && <span className="text-amber-600 text-xs font-bold">✓</span>}
                      </button>
                    );
                  })}
                </div>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-slate-100">
          ...
          <button onClick={guardar} disabled={!canSave()}
            className="flex-1 py-2.5 bg-amber-700 rounded-lg text-sm font-semibold text-white hover:bg-green-800 disabled:opacity-40 disabled:cursor-not-allowed">
            Guardar recibo
          </button>
        </div>
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      {sel ? (
        <div className="flex items-center gap-4 px-4 py-1.5 bg-blue-50 border-b border-blue-200 text-xs">
          ...
          <span className="font-bold text-amber-700">{fmtMoney(sel.monto, sel.moneda || settings.moneda || "CRC")}</span>
            ...
            {visibles.length === 0 ? (
              ...
              return (
                <tr key={r.id}
                  ...
                  <td className={`font-mono font-bold ${esAnulado ? "line-through text-slate-400" : "text-amber-700"}`}>#{r.numero}</td>
                  ...
                  <td className={`font-bold ${esAnulado ? "line-through text-slate-400" : "text-amber-700"}`}>{fmtMoney(r.monto, r.moneda || settings.moneda || "CRC")}</td>
