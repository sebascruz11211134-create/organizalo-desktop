      ...
      <tbody>
        {cuentas.length === 0 ? (
          ...
          return (
            <tr key={d.id}>
              ...
              <td className="text-green-700">{fmtMoney(d.pagado || 0, moneda)}</td>
              <td className={`font-bold ${saldo > 0 ? "text-red-600" : "text-green-700"}`}>{fmtMoney(saldo, moneda)}</td>
        ...
        {cuentas.length > 0 && (
          <tr className="bg-green-50 font-bold text-green-900 border-t-2 border-green-200">
            <td colSpan={2} className="text-green-700">TOTAL {moneda}</td>
            <td>{fmtMoney(totB, moneda)}</td>
            <td className="text-green-700">{fmtMoney(totP, moneda)}</td>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-6 px-6 py-2 bg-green-50 border-b border-green-100 text-sm">
        <span className="text-slate-500">Saldo CRC: <strong className={salCRC > 0 ? "text-red-600" : "text-green-700"}>{fmtMoney(salCRC, "CRC")}</strong></span>
        <span className="text-slate-500">Saldo USD: <strong className={salUSD > 0 ? "text-red-600" : "text-green-700"}>{fmtMoney(salUSD, "USD")}</strong></span>
