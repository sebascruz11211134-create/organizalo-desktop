  ...
  return (
    <div>
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={exportar} className="flex items-center gap-1.5 bg-emerald-700 text-white px-3 py-1.5 rounded text-xs font-semibold">
      ...
      <div className="overflow-x-auto">
        <table className="table-base w-full">
          ...
          {facturas.length > 0 && (
            <tfoot><tr className="bg-emerald-50 font-semibold">
        ...
        <table className="table-base w-full">
          ...
          <tbody>
            ...
            {compras.map((c,i) => (
              <tr key={c.id}>
                ...
                <td className="text-right text-emerald-600">{fmtMoney(c.ivaCreditoFiscal||c.iva||0,settings)}</td>
          ...
          {compras.length > 0 && (
            <tfoot><tr className="bg-emerald-50 font-semibold">
              <td colSpan={5} className="px-4 py-2 text-right text-xs text-slate-500">TOTALES</td>
              <td className="px-4 py-2 text-right">{fmtMoney(totalGravado,settings)}</td>
              <td className="px-4 py-2 text-right text-emerald-600">{fmtMoney(totalIVAcred,settings)}</td>
              <td className="px-4 py-2 text-right">{fmtMoney(totalNeto,settings)}</td>
            </tr></tfoot>
          )}
        </table>
        ...
        {TABS.map((t,i) => (
          <button key={i} onClick={()=>setTab(i)}
            className={`px-4 py-2 text-xs font-semibold rounded-t transition-colors
              ${tab===i ? "bg-emerald-700 text-white" : "text-slate-500 hover:text-slate-800 hover:bg-slate-100"}`}>
            {t}
          </button>
        ))}
      </div>
