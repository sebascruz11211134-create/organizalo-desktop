      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={guardar} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all
            ${guardado?"bg-green-600 text-white":"bg-brand-500 text-white hover:bg-brand-600"}`}>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:bg-brand-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"/>
              </label>
            </div>
            ...
            {tiendaUrl && (
              <div className="mt-4 flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                ...
                <button onClick={copiar} className="p-1 hover:bg-gray-200 rounded" title="Copiar enlace">
                  {copiado?<CheckCircle size={13} className="text-green-600"/>:<Copy size={13} className="text-slate-400"/>}
                </button>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              {[
                ...
                <label key={key} className={`block ${cls}`}>
                  ...
                  <input type={type} value={config[key]||""} onChange={e=>u(key,e.target.value)}
                    className={`mt-1 w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-brand-400 ${type==="color"?"h-10 cursor-pointer":""}`}/>
                </label>
              ))}
            ...
            <div className="flex items-center gap-3 mt-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={config.mostrarStock} onChange={e=>u("mostrarStock",e.target.checked)} className="accent-brand-500"/>
                <span className="text-sm text-slate-600">Mostrar stock disponible en la tienda</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer ml-4">
                <input type="checkbox" checked={config.moneda==="USD"} onChange={e=>u("moneda",e.target.checked?"USD":"CRC")} className="accent-brand-500"/>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              <div className="flex gap-3">
                <button onClick={whatsappLink}
                  className="flex items-center gap-2 bg-green-600 text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-green-700">
                ...
                <button onClick={copiar}
                  ...
                  {copiado?<CheckCircle size={14} className="text-green-600"/>:<Copy size={14}/>}
