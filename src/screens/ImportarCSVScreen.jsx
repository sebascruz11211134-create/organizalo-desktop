    ...
    <div className="flex flex-col h-full">
        ...
        {filas.length>0 && (
          <>
            ...
            <button onClick={importar} disabled={guardando}
              className="flex items-center gap-2 bg-amber-600 text-white px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-amber-700 disabled:opacity-60">
      ...
      {resultado && (
        <div className={`mx-6 mt-4 flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold
          ${resultado.ok?"bg-amber-50 border border-amber-300 text-amber-700":"bg-red-50 border border-red-200 text-red-700"}`}>
      ...
      {filas.length===0 ? (
        <div className={`flex-1 flex flex-col items-center justify-center gap-4 m-6 border-2 border-dashed rounded-2xl transition-colors
          ...
          <div className="text-center max-w-sm">
            ...
            <label className="mt-4 inline-flex items-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-amber-700 cursor-pointer">
              ...
              <tbody className="divide-y divide-slate-100">
                {filas.map((f,i)=>(
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className={`px-4 py-2 text-right font-bold text-sm ${f.monto>=0?"text-amber-700":"text-red-600"}`}>
                    ...
                    <td className="px-4 py-2 text-center">
                      <button onClick={()=>toggleTipo(i)}
                        className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full cursor-pointer transition-colors
                          ${f.tipo==="ingreso"?"bg-amber-100 text-amber-700 hover:bg-amber-200":"bg-red-100 text-red-600 hover:bg-red-200"}`}>
